#!/usr/bin/env python3
"""Read-only live/replay MuJoCo twin for dual-G1 ScaleBFM deployment."""
    ...
    *,
    ...
    for geom_id in range(model.ngeom):
        ...
        if geom_name is not None and "_frame_" in geom_name:
            continue
        brightness = float(np.clip(np.mean(model.geom_rgba[geom_id, :3]), 0.2, 1.0))
...
def configure_twin_appearance(model: mujoco.MjModel) -> None:
    _tint_robot(model, "actual_a_", (0.10, 0.62, 0.92), alpha=1.0, group=0)
    _tint_robot(model, "actual_b_", (0.98, 0.63, 0.12), alpha=1.0, group=0)
    _tint_robot(model, "reference_a_", (0.15, 0.95, 0.48), alpha=0.20, group=1)
    _tint_robot(model, "reference_b_", (0.92, 0.34, 0.78), alpha=0.20, group=1)
...
def apply_visualization(
    ...
    *,
    ...
    for index in range(2):
        _set_freejoint(data, int(bindings.actual_base_qpos[index]), actual_bases[index])
        ghost_base = reference_bases[index] if ghost_mode == "reference" else actual_bases[index]
        _set_freejoint(data, int(bindings.reference_base_qpos[index]), ghost_base)
        _set_mocap(data, int(bindings.tracker_mocap[index]), actual_bases[index])
    ghost_joints = (
        reference["joint_pos"]
    ...
    _set_mocap(data, int(bindings.tracker_mocap[2]), actual["object_wxyz"])
...
class ReplayClock:
    def __init__(
        ...
        *,
        ...
        self._log_anchor = float(replay.elapsed_s[self.index])

    def _reanchor(self) -> None:
        self._wall_anchor = time.monotonic()
        self._log_anchor = float(self.replay.elapsed_s[self.index])
    ...
    def step(self, delta: int) -> None:
        self.index = int(np.clip(self.index + delta, 0, self.replay.frame_count - 1))
        self.paused = True
        self._reanchor()
    ...
    def update(self) -> int:
        if self.paused or self.replay.frame_count == 1:
            return self.index
        target = self._log_anchor + (time.monotonic() - self._wall_anchor) * self.speed
        if target > self.replay.duration_s:
            ...
            return self.index
        self.index = min(
            int(np.searchsorted(self.replay.elapsed_s, target, side="right") - 1),
            self.replay.frame_count - 1,
        )
        return self.index
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "Solid blue/amber: actual A/B. Transparent green/magenta: reference. "
        "Solid/transparent boxes: measured/reference."
    )
    print("Keys: G=reference/final-PD-target ghost, Space/P=pause, N/B=step, R=restart.")
    ...
    try:
        with mujoco.viewer.launch_passive(
            ...
            viewer.cam.lookat[:] = (0.0, 0.45, 0.8)
                ...
                if now - last_status >= 1.0:
                    if replay is not None:
                        ...
                        print(
                            ...
                            f"t={replay.elapsed_s[clock.index]:.2f}s {state} ghost={ghost_mode[0]}"
                        )
                    else:
