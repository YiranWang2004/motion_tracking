...
class TensorRTBackend:
    def __init__(
        ...
        paths = [directory / "scalebfm.engine"]
        if side is not None:
            paths.append(directory / f"residual_{side}.engine")
        for path in paths:
        ...
        try:
            ...
            if len(self.info["models"]) != len(paths):
                raise ValueError("unexpected number of TensorRT models")
